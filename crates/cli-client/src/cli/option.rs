    ...
    pub(crate) async fn run_option(&self, config: Config, command: &OptionCommand) -> Result<(), Error> {
        ...
        match command {
            OptionCommand::Create {
                ...
                // Query LBTC UTXOs for fees (and collateral if LBTC)
                // Need: 2 for creation tx + 1 for funding tx fee (+ collateral if LBTC)
                let lbtc_required = if is_lbtc_collateral {
                    initial_fee * 3 + *total_collateral // creation fees (2) + funding fee + collateral
                } else {
                    initial_fee * 3 // creation fees (2) + funding fee (1)
                };
                ...
                let (collateral_outpoint, collateral_txout, funding_fee_utxo) = if is_lbtc_collateral {
                    // When collateral is LBTC, use the 3rd LBTC UTXO for collateral+fee
                    (*lbtc_entries[2].outpoint(), lbtc_entries[2].txout().clone(), None)
                } else {
                    // When collateral is not LBTC, query for collateral asset separately
                    // and use 3rd LBTC UTXO for funding fee
            ...
            OptionCommand::Exercise {
                ...
                // Pre-filter: find contracts that have collateral available
                ...
                // Filter to only ONE entry per contract where collateral exists
            ...
            OptionCommand::Settlement {
                ...
                // Pre-filter: find contracts that have settlement available
                ...
                // Filter to only ONE entry per contract where settlement exists
                // (We show one entry per contract since settlement is contract-level)
                let mut seen_contracts = std::collections::HashSet::new();
                ...
                // Query for settlement asset at contract address
                let settlement_asset_id = option_arguments.get_settlement_asset_id();
                let settlement_filter = UtxoFilter::new()
                    .taproot_pubkey_gen(taproot_pubkey_gen.clone())
                    .asset_id(settlement_asset_id);
            ...
            OptionCommand::Cancel {
                ...
                // Find grantor token with matching contract
                ...
                // Determine amount to burn (minimum of both token amounts)
                let option_token_amount = option_entry.value().unwrap_or(0);
                let grantor_token_amount = grantor_entry.value().unwrap_or(0);
                ...
                // Find collateral UTXO at contract address
                let collateral_filter = UtxoFilter::new()
                    .taproot_pubkey_gen(taproot_pubkey_gen.clone())
                    .asset_id(option_arguments.get_collateral_asset_id());
                ...
                // Finalize contract input (index 0) with Simplicity witness
                let options_program = get_options_program(&option_arguments)?;
                tx = finalize_options_transaction(
                    tx,
                    &taproot_pubkey_gen.get_x_only_pubkey(),
                ...
                // Sign P2PK inputs (indices 1, 2, 3)
                let tx = sign_p2pk_inputs(tx, &utxos, &wallet, config.address_params(), 1)?;
