...
#[must_use]
pub fn option_offer_created() -> Filter {
    Filter::new().kind(OPTION_OFFER_CREATED)
}

#[must_use]
pub fn option_offer_created_by_pubkey(pubkey: nostr::PublicKey) -> Filter {
    Filter::new().kind(OPTION_OFFER_CREATED).author(pubkey)
}
